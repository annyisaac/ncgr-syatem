...
export default function AvailabilityPage() {
  ...
  const accRange = useMemo(() => presetToRange(preset, ALL_TIME, todayISO()), [preset]);
  const accuracy = useMemo(() => {
    const list = hatched.filter((h) => inRange(h.hatchDate, accRange));
    ...
    return { list, eggsSet, projected, actual, realPct: eggsSet > 0 ? Math.round((actual / eggsSet) * 100) : 0, variance: projected > 0 ? Math.round((actual / projected) * 100) - 100 : 0 };
  }, [hatched, accRange]);
  ...
  function exportAccuracy() {
    ...
    const el = document.createElement("a"); el.href = url; el.download = "projection-accuracy.csv"; el.click(); URL.revokeObjectURL(url);
  }
  ...
  return (
    <div className="space-y-5">
        ...
        <Card>
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" variant="ghost" onClick={exportAccuracy}>⬇ Export</Button>
            </div>
          </div>
      ...
      <Card>
        <div id="dates-table" className="flex flex-wrap items-start justify-between gap-3 scroll-mt-24">
          <div className="flex items-start gap-3">
...
// ---- projection-accuracy tiles + icons -------------------------------------
